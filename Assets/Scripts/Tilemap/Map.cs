...
namespace ProjectQuad
{
    ...
    public class Map : MonoBehaviour
    {
        ...
        HeightMap heightMap;
        IMapMeshGenerator meshGenerator;

        readonly JsonDataService dataService = new();

        public void InitializeMap()
        {
            heightMap ??= LoadMapData();
            mapSize = heightMap.GetMapSize();
            meshGenerator = GetComponent<IMapMeshGenerator>();
        ...
        public void PlaceTiles(List<Vector2Int> coords, int height)
        {
            ...
            var editedChunks = GetEditedChunks(editedTiles);
            RegenerateChunks(editedChunks);
            SaveMapData();
        }
        ...
        public void RegenerateChunks(HashSet<Vector2Int> editedChunks)
        {
            foreach (Vector2Int chunkCoord in editedChunks)
            {
                if (chunkCoord.x < 0 || chunkCoord.x > (mapSize.x - 1) / chunkSize ||
                    chunkCoord.y < 0 || chunkCoord.y > (mapSize.y - 1) / chunkSize) 
                    continue;

                Debug.Log($"Chunk {chunkCoord} has been regenerated");
                meshGenerator.GenerateChunkMesh(chunkCoord, heightMap);
            }
        }
        ...
        public void ReloadMap()
        {
            ...
            meshGenerator.RecalculateTilesetUV();
            meshGenerator.GenerateMapMesh(heightMap);
        }
