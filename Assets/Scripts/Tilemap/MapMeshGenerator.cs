...
{
    [ExecuteInEditMode]
    public class MapMeshGenerator : MonoBehaviour, IMapMeshGenerator
    {
            ...
            switch (bitmask)
            {
                ...
                case 3:
                    ...
                    PlaceDiagWallTiles(tileCoord, wallStartSW.y, tileHeight,
                        new Tile[] { GetTile(RuleTile.WALL_SW_BASE), GetTile(RuleTile.WALL_SW), GetTile(RuleTile.WALL_SW_TOP) });
                    break;
                ...
                case 6:
                    ...
                    {
                        PlaceGroundTile(new Vector3(tileCoord.x, neighbours[3], tileCoord.y), GetTile(RuleTile.EDGE_N));
                    }
                    else
                    {
                ...
                case 9:
                    ...
                    PlaceDiagWallTiles(tileCoord, wallStartSE.y, tileHeight, 
                        new Tile[] { GetTile(RuleTile.WALL_SE_BASE), GetTile(RuleTile.WALL_SE), GetTile(RuleTile.WALL_SE_TOP) });
                    break;
                ...
                case 12:
                    ...
                    else if (neighbours[1] > neighbours[0])
                    {
                        PlaceGroundTile(new Vector3(tileCoord.x, neighbours[1], tileCoord.y), GetTile(RuleTile.EDGE_N));
                    }
                    else
                    {
                        PlaceGroundTile(new Vector3(tileCoord.x, neighbours[0], tileCoord.y), GetTile(RuleTile.GROUND));
        ...
        private void PlaceDiagWallTiles(Vector2Int tileCoord, int baseHeight, int topHeight, Tile[] tiles)
        {
            if (tiles.Length != 3)
                throw new MissingReferenceException("PlaceDiagWallTiles needs 3 tiles to be passed");

            meshData.AddMesh(new Vector3(tileCoord.x, baseHeight, tileCoord.y), tiles[0]);
            for (int height = baseHeight + 1; height < topHeight; height++)
            {
                meshData.AddMesh(new Vector3(tileCoord.x, height, tileCoord.y), tiles[1]);
            }
            meshData.AddMesh(new Vector3(tileCoord.x, topHeight, tileCoord.y), tiles[2]);
        }
